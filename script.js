...
function renderMessages() {
  ...
  messages.slice(1).forEach((msg) => {
    ...
    div.innerHTML = `<span class=\"fw-bold\">${
      msg.role === "user" ? "You" : "Assistant"
    }:</span> <span>${msg.content.replace(/\n/g, "<br>")}</span>`;
...
async function getResponse(prompt) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        err.error || "An error occurred while connecting to the server"
      );
    }
...
chatForm.addEventListener("submit", async (e) => {
  ...
  if (reply) {
    ...
    if (messages.length > MAX_HISTORY + 1)
      messages.splice(1, messages.length - MAX_HISTORY - 1);
    renderMessages();
  }
});
